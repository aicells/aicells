...
class aicSLModelMetricsCV(AICFunction):
    def Run(self, arguments):
        ...
        if not (a['selected_features'] is None):
            for column in a['selected_features']:
                ...
                if not (column in input_data.columns):
                    raise AICException.AICEParameterError("PARAMETER_UNKNOWN_COLUMN", {"parameterName": 'selected_features',
        ...
        try:
            kf = sklearn.model_selection.KFold(n_splits=a['n_splits'])
        except Exception as e:
            errorMessage = ""
            if len(e.args) > 0:
        ...
        for train_index, test_index in kf.split(X):
            self.Progress(f"Cross validation round {i} from {a['n_splits']}")
            i = i + 1
            ...
            for selectedMetric in metricList:
                ...
                for metric in self.y['metrics']:
                    if metric['name'] == selectedMetric:
                        ...
                        function = metric['function'].split('(')[0].split(
                            '.')  # eg. sklearn, metrics, median_absolute_error
                        f1 = getattr(sklearn, function[1])  # eg. metrics
                        f2 = getattr(f1, function[2])  # eg. median_absolute_error

                        try:
                            rowDict[("train_" + selectedMetric).replace('_', ' ')] = sign * f2(y_train, y_pred_train,
                                                                                               **kwargs)
                            ...
                            if len(e.args) > 0:
                                errorMessage = e.args[0]
                            raise AICException.AICException("METRIC_ERROR", {"error": errorMessage, 'metric': selectedMetric})

                if not (("train_" + selectedMetric).replace('_', ' ') in rowDict):
                    raise AICException.AICException("UNKNOWN_METRIC", {'scorer': selectedMetric})
