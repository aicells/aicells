...
class aicFunctionParameterDescriptions(AICFunction):
    def Run(self, arguments):
        ...
        try:
            ...
            l = c.GetParameterDescritpionList()
            if c.GetParameterNameList()[0] == 'parameters':
                l.pop(0)
            if a['show_function_and_output']:
                l = ['name of the AIcells function (only required, when the function is started from the ribbon)',
                     'where to place the results (only required, when the function is started from the ribbon)'] + l
