...
class aicRandom(AICFunction):
    def Run(self, arguments):
        ...
        try:
            randomArray = tool.GetRandomArray(toolA)
        except Exception as e:
            errorMessage = ""
            if len(e.args) > 0:
                errorMessage = e.args[0]
            raise AICException.AICException("TOOL_ERROR", {"error": errorMessage})

        return UDFUtils.ReturnList(randomArray.tolist())
